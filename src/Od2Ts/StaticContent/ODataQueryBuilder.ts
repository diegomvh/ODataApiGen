...
export const buildEntityKey = (key) => {
  if (typeof(key) === 'object')
    return Object.keys(key).map(k => `${k}=${key[k]}`).join(',');
  return `${key}`;
}

export const buildAction = (action) => `${action}`;

export const buildFunction = (func) => {
  let segment;
...
export const buildFilter = (filters = {}, propPrefix = '') => {
  if (filters == null) {
...
export const buildExpand = (expands) => {
...
export const buildTransforms = (transforms) => {
...
export const buildAggregate = (aggregate) => {
...
export const buildGroupBy = (groupBy) => {
